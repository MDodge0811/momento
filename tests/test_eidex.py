...
class TestEidex:
    ...
    def test_ensure_db_creates_database(self):
        ...
        # Check index
        cursor.execute("SELECT name FROM sqlite_master WHERE type='index' AND name='idx_branch_timestamp'")
        assert cursor.fetchone() is not None
    ...
    def test_get_current_branch_outside_git(self):
        ...
        with patch('eidex.Repo') as mock_repo_class:
            ...
            with pytest.raises(ValueError, match="Not in a Git repository. Eidex requires a Git repo."):
    ...
    def test_log_work(self):
        ...
        conn = sqlite3.connect(db_path)
        cursor = conn.cursor()
        cursor.execute("SELECT message, branch, extra FROM logs WHERE message = 'Test message'")
        row = cursor.fetchone()
    ...
    def test_prune_old_logs(self):
        ...
        cursor.execute("INSERT INTO logs (timestamp, branch, message, extra) VALUES (?, ?, ?, ?)",
...
class TestEidexCLI:
    ...
    def test_cli_log_work(self):
        ...
        eidex_script = os.path.join(os.path.dirname(os.path.dirname(__file__)), 'eidex.py')
    ...
    def test_cli_log_work_with_extra(self):
        ...
        eidex_script = os.path.join(os.path.dirname(os.path.dirname(__file__)), 'eidex.py')
        result = os.system(f'python3 {eidex_script} log_work "CLI test message" --extra \'{extra_json}\'')
        assert result == 0

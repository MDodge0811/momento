...
def get_repo_root():
    ...
    try:
        ...
        raise ValueError("Not in a Git repository. Momento requires a Git repo to store logs.")
...
def ensure_db():
    ...
    ''')
    cursor.execute('CREATE INDEX IF NOT EXISTS idx_branch_timestamp ON logs (branch, timestamp)')
    conn.commit()
    conn.close()
...
def log_work(message: str, extra_info: dict = None):
    ...
    cursor = conn.cursor()
    cursor.execute('INSERT INTO logs (branch, message, extra) VALUES (?, ?, ?)',
...
def main():
    ...
    fetch_parser.add_argument('--branch', help='Branch name (default: current)', default=None)
    fetch_parser.add_argument('--limit', type=int, help='Max logs to return', default=50)

    # cleanup_deleted_branches command
    subparsers.add_parser('cleanup_deleted_branches', help='Delete logs for non-existent branches')
